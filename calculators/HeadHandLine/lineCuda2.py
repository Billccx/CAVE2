...
class LineCuda2(BaseCalculator):
    def __init__(self,path):
        f = open('D:/CCX/Pipeline/extrinsics/' + path, 'r', encoding='utf-8')
        R_ = []
        t_ = []
        knt = 0
        for line in f:
    ...
    def Process(self,img,**kwargs):
        ...
        if len(faceresult.result):
            ...
            if int(headpoint2d[0])>=0 and int(headpoint2d[0])<640 and int(headpoint2d[1])>=0 and int(headpoint2d[1])<480: #change more elegant way
                ...
                print("headpoint_depth: {}".format(headpoint_depth))
        ...
        if (handsresult.result !=None) and len(handsresult.result):
            ...
            if int(handpoint2d[0]) >= 0 and int(handpoint2d[0]) < 640 and int(handpoint2d[1]) >= 0 and int(handpoint2d[1]) < 480:
                ...
                print("handpoint_depth: {}".format(handpoint_depth))
        ...
        isOccluded=False
        # if headpoint3d[2][0]!=0 and handpoint3d[2][0]!=0 and abs(headpoint3d[2][0]-handpoint3d[2][0])<0:
        #     print("__________________________________________________________________________________")
        #     isOccluded=True

        result=Result('linecuda')
        result.setResult((x,y,isOccluded))
    ...
    def Draw(self,img,**kwargs):
        print('fuck! this function should not be called')
        ...
        if canShow:
            if (kwargs['index'] == 0):
                #cv2.circle(img=img, center=(int(x)+10, int(y)+10), radius=30, color=(0, 255, 255), thickness=cv2.FILLED)
                pass
            else:
                #cv2.circle(img=img, center=(int(x), int(y)), radius=30, color=(255, 255, 0), thickness=cv2.FILLED)
                #print("in Line cuda, (255, 255, 0) coordinate ({},{})".format(x, y))
                pass
