...
class Line3(BaseCalculator):
    ...
    def Process(self,img,**kwargs):
        ...
        :param kwargs: depthframe, intrinsics, faceresult, handposeresult
        :return:
        '''
        depth=kwargs['depthframe']
        intrinsics=kwargs['intrinsics']
        faceresult=kwargs['faceresult']
        handposeresult=kwargs['handposeresult']
        ...
        handpoint3d = [0, 0, 0]
        if handposeresult.result:
            handslandmark=handposeresult.result
            [x, y] = handslandmark
            if int(x) >= 0 and int(x) < 640 and int(y) >= 0 and int(y) < 480:
                handpoint_depth = depth.get_distance(int(x),int(y))
                handpoint3d=rs2.rs2_deproject_pixel_to_point(intrin=intrinsics,pixel=(int(x),int(y)),depth=handpoint_depth)
                hasHand = True
