...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='./weights/resnet_50-size-256-wingloss102-0.119.pth',
                        help='model_path')  # 模型路径
    ...
    with torch.no_grad():
        ...
        cams.captureRGB(0)
        hands = HandsDectYolo()
        cap = cv2.VideoCapture('basicvideo4.mp4')
        ...
        #while cap.isOpened():
            ...
            if len(handsresult.result):
                handsresult.result.sort(key=lambda x: x[2])
